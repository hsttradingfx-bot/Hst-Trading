            ...
            for m in range(p_conf['b_idx'] + 1, len(candles_m1)):
                ...
                if p_h1['sens'] == 'haussier':
                    ...
                    if c_check.high >= (sl + ajustement) or c_check.low <= (tp + ajustement):
                        ...
                        break
            if statut == "EXPIRE":
                log_rejet(symbol, p_h1['sens'], ts_val_conf, "signal caduc (SL/TP déjà touché)")
                continue

            cle = f"{symbol}_{p_h1['b_idx']}_{p_conf['b_idx']}"
            if cle in state:
                log_rejet(symbol, p_h1['sens'], ts_val_conf, "déjà alerté")
                continue
...
def formater_message(sig, execution=None):
    ...
    base = (
        f"{emoji} <b>[M1 SCALPING] {sig['symbol']} - {direction}</b>\n"
