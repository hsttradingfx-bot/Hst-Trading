...
# MAX_ATTENTE_MINUTES réel = 24h, trop long pour boucler en continu à 30s sans interruption
# (ça ferait ~2880 fetch M1 par actif rien que pour une seule attente). On limite la boucle rapide
# à une fenêtre plus réaliste — au-delà, le cron normal (5 min) reprend la main et continue de détecter
# le signal, juste avec un délai de 5 min au lieu de 30s.
DUREE_MAX_ATTENTE_RAPIDE_MINUTES = 240
DUREE_MAX_BOUCLE_SEC = (DUREE_MAX_ATTENTE_RAPIDE_MINUTES + 20) * 60
...
def surveiller_positions_ouvertes(state, candles_par_symbole):
    ...
    for cle, pos in positions.items():
        ...
        if touche:
            ...
            if resultat["succes"]:
                envoyer_telegram(f"{emoji} [RETRACEMENT] {symbol} [{pos['sens']}] : {touche} touché, clôture envoyée à Gainium.")
            else:
                envoyer_telegram(f"⚠️ [RETRACEMENT] {symbol} [{pos['sens']}] : {touche} touché mais échec de clôture Gainium ({resultat['erreur']}) — vérifie manuellement.")
